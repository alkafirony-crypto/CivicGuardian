  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 px-5 py-8 sm:px-8 sm:py-10">
      ...
      <section className="overflow-hidden rounded-2xl border border-[#514c48] bg-[#2c2927] p-5 text-white shadow-xl sm:p-7">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="flex items-center gap-2 text-[13px] font-semibold uppercase tracking-[.12em] text-[#e0ff89]"><Award className="h-4 w-4" />Community heroes</div><h2 className="mt-2 text-2xl font-bold">Top contributors</h2><p className="mt-2 leading-6 text-slate-300">Points come only from real reports and community actions. Administrator workflow updates never earn points.</p></div><button type="button" onClick={() => setActiveTab("heroes")} className="rounded-lg border border-[#5bcff4] px-4 py-2.5 font-semibold text-[#bdecfb] hover:bg-[#5bcff4]/10">View full leaderboard →</button></div>
        {heroes.length ? <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{heroes.slice(0, 8).map((hero, index) => <div key={hero.id} className="rounded-lg border border-[#625c57] bg-[#383431] p-3"><div className="flex items-center gap-2.5"><span className="grid h-8 w-8 place-items-center rounded-full bg-sky-400/10 text-[10px] font-black text-[#e0ff89]">#{index + 1}</span><div className="min-w-0"><div className="flex items-center gap-2"><div className="truncate text-xs font-extrabold text-slate-100">{hero.name}</div>{hero.role === "admin" && <span className="rounded bg-violet-500/15 px-1.5 py-0.5 text-[8px] font-black uppercase text-violet-300">Admin</span>}</div><div className="text-[9px] font-bold text-slate-500">{hero.score} contribution points</div></div></div></div>)}</div> : <div className="mt-4 rounded-lg border border-dashed border-slate-700 p-6 text-center text-xs text-slate-500">No contributor ranking yet. CivicGuardian will not invent leaderboard entries.</div>}
      </section>
