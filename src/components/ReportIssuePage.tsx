    ...
    <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-10">
      ...
      <div className="grid gap-8 lg:grid-cols-[260px_1fr]">
        <aside>
          ...
          <p className="mt-3 text-[15px] leading-6 text-slate-600">Add the location and real evidence. Gemini assists with classification, while people control verification and resolution.</p>
          <div className="mt-5 grid grid-cols-3 gap-2 lg:grid-cols-1">
            {[["location", "1", t("location")], ["evidence", "2", t("evidence")], ["review", "3", t("review")]].map(([id, number, label]) => (
              <div key={id} className={`flex min-w-0 items-center gap-3 rounded-xl p-3 text-[14px] font-semibold ${phase === id ? "bg-[#e0ff89] text-[#2c2927]" : "text-slate-500"}`} aria-current={phase === id ? "step" : undefined}>
                <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full text-[13px] ${phase === id ? "bg-[#2c2927] text-white" : "bg-slate-200"}`}>{number}</span><span className="truncate">{label}</span>
              </div>
            ))}
          </div>
        </aside>
        ...
        <section>
          ...
          {phase === "evidence" && <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
            ...
            <div className="mt-6 flex gap-3"><button type="button" onClick={() => setPhase("location")} className="rounded-lg border border-slate-300 px-5 py-3 font-semibold text-slate-700">Back</button><button type="button" disabled={busy || !online || !image || description.trim().length < 10 || !evidenceConsent} onClick={() => void analyze()} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-[#2c2927] px-5 py-3 font-semibold text-white hover:bg-[#413d39] disabled:opacity-40"><Sparkles className="h-4 w-4 text-[#e0ff89]" />{busy ? t("analyzing") : t("analyze")}</button></div>
          </div>}

          {phase === "review" && analysis && <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
