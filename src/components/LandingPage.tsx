...
export default function LandingPage({
  ...
  const publicAreas = [
    [MapPinned, "Map & Reports", "Public", "Explore citizen reports and the Dhaka hazard map.", "dashboard"],
  ...
  return (
    <div>
      <section className="relative overflow-hidden border-b border-[#223149] bg-[#0b1220] text-white">
        ...
        <div className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 select-none text-[clamp(120px,20vw,310px)] font-black tracking-[-.08em] text-white/[.018]">
          DHAKA
        </div>

        <div className="relative mx-auto grid max-w-7xl items-center gap-8 px-5 py-11 sm:px-6 lg:grid-cols-[1.08fr_.92fr] lg:py-16">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-sky-400/20 bg-sky-400/[.07] px-3 py-1.5 text-[11px] font-bold text-sky-200">
              <ShieldCheck className="h-3.5 w-3.5" /> Community hazard reporting across Dhaka
            </div>
            <h1 className="mt-5 max-w-3xl text-[2.35rem] font-black leading-[1.04] tracking-[-.04em] text-white sm:text-[3rem]">
              See a public hazard?<br />
              <span className="text-sky-300">Report what is really there.</span>
            </h1>
            <p className="mt-4 max-w-2xl text-[14px] leading-6 text-slate-300">
              CivicGuardian helps Dhaka residents submit real photos and precise locations for road, drainage, electrical, waste and public-safety issues. AI assists with classification; people remain in control.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <button onClick={() => setActiveTab("report")} className="flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-3 text-[13px] font-extrabold text-white shadow-lg shadow-sky-950/20 transition hover:-translate-y-0.5 hover:bg-sky-500">
                Report a hazard <ArrowRight className="h-4 w-4" />
              </button>
              <button onClick={() => setActiveTab("dashboard")} className="flex items-center gap-2 rounded-lg border border-slate-600 bg-white/[.04] px-4 py-3 text-[13px] font-bold text-slate-200 transition hover:border-sky-400/50 hover:bg-white/[.07] hover:text-white">
                <MapPinned className="h-4 w-4 text-sky-300" /> Explore Dhaka map
              </button>
            </div>
          ...
          <div className="rounded-2xl border border-[#273850] bg-[#111b2b]/90 p-5 shadow-2xl shadow-black/20 backdrop-blur sm:p-6">
            <div className="text-[10px] font-black uppercase tracking-[.17em] text-sky-300">Dhaka service area</div>
            <h2 className="mt-2.5 text-2xl font-black tracking-tight text-white">One map. Any supported hazard. Any valid Dhaka location.</h2>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2.5">
              {[[MapPinned, "Dhaka-wide map"], [Camera, "Real uploads"], [BrainCircuit, "AI advisory"], [Users, "Community checks"]].map(([Icon, text]: any) => (
                <div key={text} className="rounded-xl border border-slate-700/80 bg-slate-950/25 p-3.5 transition hover:-translate-y-0.5 hover:border-sky-400/30">
                  <Icon className="h-[18px] w-[18px] text-sky-300" />
                  <div className="mt-2.5 text-[13px] font-bold text-slate-100">{text}</div>
                </div>
              ))}
            </div>
            <div className="mt-4 rounded-xl border border-slate-700/70 bg-slate-950/25 px-4 py-3 text-[11px] leading-5 text-slate-400">
      ...
      <section className="border-b border-slate-200 bg-[#f7f9fb]">
        <div className="mx-auto max-w-7xl px-5 py-10 sm:px-6">
          ...
          <div className={`mt-6 grid gap-3 md:grid-cols-2 ${platformAreas.length >= 4 ? "xl:grid-cols-4" : "xl:grid-cols-2"}`}>
      ...
      <section className="mx-auto max-w-7xl px-5 py-12 sm:px-6">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-3 text-3xl font-black tracking-tight text-slate-950">Evidence first, then AI assistance.</h2>
        </div>
        <div className="mt-9 grid gap-5 md:grid-cols-3">
          {[
            [Camera, "1. Capture evidence", "Upload or take a real photo and select the exact Dhaka location. No generated incident imagery is used."],
            ...
            <div key={title} className="rounded-2xl border border-slate-200 bg-white p-6 transition hover:-translate-y-0.5 hover:shadow-lg">
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-sky-50 text-sky-700"><Icon className="h-5 w-5" /></span>
        ...
        <div className="mx-auto max-w-7xl px-5 py-14 sm:px-6">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black text-slate-950">Recent citizen reports</h2>
            </div>
          ...
          {recentIssues.length ? (
            <div className="mt-7 grid gap-5 md:grid-cols-3">
              {recentIssues.slice(0, 3).map(issue => (
                <button key={issue.id} onClick={() => { setSelectedIssue(issue); setActiveTab("detail"); }} className="overflow-hidden rounded-2xl border border-slate-200 bg-white text-left transition hover:-translate-y-0.5 hover:shadow-lg">
                  <IssueImage src={issue.imageUrl} alt={`Citizen evidence for ${issue.title}`} className="h-40 w-full object-cover" />
                  <div className="p-4">
